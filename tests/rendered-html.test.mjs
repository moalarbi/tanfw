...
test("يحافظ على محتوى ملف وورد داخل بيانات المشروع", () => {
  assert.equal(content.stats.sections, 35);
  assert.equal(content.stats.tables, 22);
  assert.equal(content.stats.textItems, 824);
  assert.equal(
    content.contentHash,
    "c5b75197f210c2c564cdaf75e6c2ab02cdafe340ce427d6ead6a234a7ad4a537",
  );
  assert.equal(
...
test("يحذف أقسام التوقعات المالية المعتمدة", () => {
  ...
  const removedSectionNumbers = /^(28|29|30|31|33|39|40)\./;
...
test("يطبق نطاقات الرصيد التشغيلي المعتمدة", () => {
  ...
  assert.match(serializedContent, /2,630,000 – 3,630,000 ريال/);
  assert.match(serializedContent, /9,160,000 – 10,660,000 ريال/);
});
