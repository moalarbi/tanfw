...
test("يحافظ على محتوى ملف وورد داخل بيانات المشروع", () => {
  ...
  assert.equal(
    content.contentHash,
    "bc34a69312db5d9b26447d5100f5565b0a0556c6f9282776cfdd4aaac3545659",
  );
  assert.equal(
...
test("يعرض نص وحدة كونسيرج الأعمال والتنفيذيين المعتمد", () => {
  const serializedContent = JSON.stringify(content);

  assert.match(serializedContent, /ج\. وحدة كونسيرج الأعمال والتنفيذيين/);
  assert.match(serializedContent, /تدير هذه الوحدة الاحتياجات المهنية والتنفيذية/);
  assert.match(serializedContent, /توفير نقطة اتصال موحدة لمتابعة جميع التفاصيل حتى اكتمال التنفيذ/);
});

test("يعرض بنود وحدة نمط الحياة والكونسيرج المعتمدة", () => {
  const serializedContent = JSON.stringify(content);

  assert.match(serializedContent, /خدمات التسوق الشخصي\./);
  assert.match(serializedContent, /تنسيق خدمات العناية والاستجمام والعافيه\./);
  assert.match(serializedContent, /ترتيب الوصول إلى التجارب والخدمات محدودة الإتاحة\./);
  assert.doesNotMatch(serializedContent, /موعد العافية والرفاه\./);
});
